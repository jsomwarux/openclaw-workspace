...
3. **Load Voice Profile:** Read `~/.openclaw/workspace/memory/content/jt-voice-profile.md`. Use it for taste-level decisions: buyer-scene hooks, rejected patterns, proof-safe ownership, and the pre-send voice gate. Until the profile includes the Tier 1 public JT-written post inventory listed in its Missing Evidence section, treat it as a correction-derived guardrail, not a full voice clone.
