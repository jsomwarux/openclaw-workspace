---
name: ai-seo
description: "Optimize a website or page to appear in AI-generated answers (Google AI Overviews, ChatGPT, Perplexity, Claude, Gemini). Use when JT says 'AI SEO,' 'GEO,' 'LLM SEO,' 'optimize for AI search,' 'show up in Perplexity,' 'get cited by ChatGPT,' 'AI visibility,' 'llms.txt,' 'answer engine optimization,' or wants to improve how jtsomwaru.com or a client site ranks in AI answers. Also use when this could become a consulting deliverable for SMB clients. For traditional technical SEO, this skill also covers foundational signals that feed AI citation."
---

# AI SEO — Generative Engine Optimization (GEO)

You are an expert in AI search optimization. Your goal is to help content get discovered, extracted, and cited by AI systems including Google AI Overviews, ChatGPT, Perplexity, Claude, Gemini, and Copilot.

## Before Starting

Read MEMORY.md for JT's context:
- **jtsomwaru.com** already has: llms.txt, JSON-LD schema, open AI crawler access (implemented 2026-03-11)
- **Target queries for JT's site**: "AI consultant NYC," "n8n automation consultant," "Agentforce implementation," "AI workflow automation small business"
- **Consulting services**: n8n ($3,500), Agentforce ($6,500), AI App Development ($4,500)
- **This skill can also be a consulting deliverable** — SMB clients want to appear in AI search for their niche

---

## How AI Search Works

| Platform | Source Selection |
|----------|-----------------|
| Google AI Overviews | Strong correlation with traditional rankings — fix SEO first |
| ChatGPT (with search) | Draws from wider range, not just top-ranked |
| Perplexity | Always cites sources — favors authoritative, recent, well-structured |
| Gemini | Google index + Knowledge Graph |
| Claude | Training data + Brave search results |

**Key difference from traditional SEO:** Traditional SEO gets you ranked. AI SEO gets you **cited**.

AI systems look for:
1. **Authoritative answers** to specific questions
2. **Structured, extractable content** (clear headings, lists, direct answers)
3. **Entity clarity** — is it unambiguous who/what you are?
4. **Citation worthiness** — would a human researcher cite this source?

---

## The GEO Optimization Framework

### Tier 1: Foundation (must have before anything else)
- [ ] **llms.txt** — tells AI crawlers how to understand your site. Already done for jtsomwaru.com.
- [ ] **JSON-LD schema** — Person, Organization, Service, FAQPage schemas. Already done for jtsomwaru.com.
- [ ] **Open AI crawler access** — robots.txt allows GPTBot, ClaudeBot, PerplexityBot. Already done.
- [ ] **Clean canonical URLs** — no duplicate content issues
- [ ] **Page speed** — slow pages get deprioritized

### Tier 2: Content Structure
AI systems extract answers. Structure content so the answer is obvious:

**Direct answer format (what Perplexity loves):**
```
## What is [topic]?
[Direct 1-2 sentence answer]

[Expanded explanation in short paragraphs]

[Bullet points for key facts]
```

**FAQ sections** — literally answer the question in the heading, then answer it in the first sentence of the body.

**Definition blocks** — for any technical term or service you offer, define it clearly in one sentence.

### Tier 3: Entity Establishment
AI systems need to understand who JT is and what he does:
- Consistent Name + Location + Specialty across all online profiles
- Wikipedia-style clear first paragraph: "Jon Trevor Somwaru is an AI consultant based in NYC specializing in n8n workflow automation and Salesforce Agentforce implementation for small businesses."
- LinkedIn, X bio, jtsomwaru.com About section — all should describe the same person the same way

### Tier 4: Citation Signals
Being cited by others trains AI systems to cite you:
- Guest posts and mentions on industry sites
- Being referenced in subreddit discussions
- Directory listings (Futurepedia, AlternativeTo, G2 for consulting tools)
- LinkedIn articles that get shared and linked

---

## Content Formats That Get Cited

**High citation probability:**
1. **Definitive guides** — "The Complete Guide to n8n for Small Business" (long, comprehensive, regularly updated)
2. **Original data/research** — "I analyzed 50 NYC SMB automation projects — here's what I found"
3. **Comparison content** — "n8n vs Make.com for construction companies" (comparative, specific)
4. **Process documentation** — "How to set up Agentforce for property management" (step-by-step)
5. **FAQ pages** — answer 20+ real questions people ask in your niche

**Low citation probability:**
- Generic marketing copy
- Content that could apply to anyone
- Vague claims without specifics
- Content without a clear author/entity

---

## For jtsomwaru.com Specifically

**Current gaps to fix:**
1. Add a dedicated `/services/n8n-automation` page with structured FAQ content
2. Add a dedicated `/services/agentforce` page with comparison content (Agentforce vs. hiring an ops person)
3. Add case study pages: `/case-studies/aya-construction-dashboard` — real outcomes, real numbers
4. Add FAQ section to homepage answering: "What does an AI consultant do?" "How much does automation cost for a small business?" "What's the ROI of n8n automation?"

**Queries to target for each page:**
- Homepage: "AI consultant NYC," "AI automation consultant small business"
- n8n page: "n8n automation for construction," "workflow automation for contractors"
- Agentforce page: "Agentforce implementation consultant," "Salesforce Agentforce setup"
- Case studies: "AI dashboard for construction," "StreetEasy automation"

---

## As a Consulting Deliverable

**AI Visibility Audit** — billable to SMB clients:
1. Check if client appears in ChatGPT, Perplexity, Gemini for their 5 target queries
2. Audit robots.txt for AI crawler access
3. Check for JSON-LD schema on key pages
4. Identify content gaps (no FAQ, no structured answers)
5. Deliver a one-page report with 5 prioritized fixes

Price anchor: $500–$1,500 one-time audit. Can be included as a bonus in larger n8n or Agentforce engagements to increase perceived value.

---

## Monitoring AI Visibility

Manually check monthly:
- ChatGPT: "Who are the best AI consultants in NYC?" "Best n8n consultants"
- Perplexity: same queries
- Google AI Overviews: "n8n automation for small business"

If not appearing: identify which Tier is missing and fix it.
If appearing: screenshot it — this is proof of work for the site and for client pitches.
