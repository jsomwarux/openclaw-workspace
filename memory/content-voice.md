...
|---|---|---|---|---|
| 2026-09-23 | Cohort-Two Verified Seed Roster v2 | Merged n8n-agent PR #16 as `7863deee` after independent pre-merge and post-merge clean-clone gates each passed 623 tests with zero failures/skips. The roster retains 20 evidence-bearing organizations, including 13 new verified providers; its exact bytes and reviewed 58/4-node workflows are now deployed inactive with schedule, Mission Control, heartbeat, and send paths still disabled. | Evidence-backed prospect-universe expansion, adversarial host validation, generated n8n source discipline, verifier-gated Claude integration | Complete / deployed inactive |
| 2026-09-23 | Cohort-Two Bounded Two-Page Inactive Deployment | Deployed exact `n8n-agent` merge `41ef3bfb` into live n8n as a 58-node main and 4-node error workflow; both remain inactive, all three external-action nodes are disabled, zero send-capable nodes exist, 11 unrelated active workflows are unchanged, runtime health is 200, and no new cohort execution occurred. | Recoverable n8n deployment, source/runtime identity proof, bounded continuation, least-privilege release control | Complete / inactive / internal proof only |
| 2026-09-17 | Cohort-Two n8n Discovery Inactive Deployment | Merged n8n-agent PR #7 as `1e37f62`, deployed the 57-node main and 4-node error workflow with D3 restrictions, passed 261 tests and a 35-node loopback-only runtime execution, preserved 11 active workflows, and deleted six authorized inactive probes. Both cohort workflows remain inactive with zero send-capable nodes. | n8n systems architecture, fail-closed deployment, fixture-backed runtime proof, least-privilege operations | Complete / inactive / internal proof only |
| 2026-09-16 | Growth OS Canonical Proof + n8n Build Blueprint | Bound JT-approved Option B to merged COI live-system evidence, merged the canonical proof card as `727213ba` with 10/10 checks green, then fresh-verified and merged the reconciled 56-node n8n blueprint as `fab6c62c`; all activation/send boundaries remain off. | Evidence-bound claims, human attestation, cross-repo contract reconciliation, fail-closed n8n design, independent verification | Complete / inactive / internal proof only |
| 2026-09-16 | Mission Control Suppression Owner | Merged PR #6 at authorized head `d9beca4` and deployed the reviewed suppression owner inactive; 33 focused TypeScript tests, 8 Python tests, the isolated production build, live Convex function metadata, and fail-closed HTTP 503 behavior passed with `OUTREACH_SUPPRESSION_OWNER_ENABLED` unset. | Fail-closed feature deployment, immutable workflow evidence, least-privilege boundaries, release verification | Complete / inactive / internal proof only |
| 2026-09-15 | Cohort-Two Verified-Channel Owner Adapter | PR #40 merged as `d015aa43`; the adapter accepts exactly one currently published email from the Git-bound organization contact surface, fails closed on ambiguity/unsafe network or URI cases, and exact post-merge workflow `35041738234` passed all ten jobs. It remains inactive and has no send authority. | SSRF-resistant integration design, evidence ownership, hostile parser validation, approval-gated automation | Complete / internal proof only |
| 2026-09-15 | Mission Control Universal Action Cards | PR #4 merged as `68757613` and deployed typed exact steps, paste-ready prompt, paste/use destination, done state, and append-only feedback; 204 tests, TypeScript, the 41-route build, and a live create/feedback/hostile-request/delete proof passed with the active-task count restored to 47. | Backward-compatible schema evolution, action-card UX, append-only human feedback, safe API validation, live deployment verification | Complete / internal proof only |
| 2026-09-15 | Mission Control Immutable Outreach Review Contract | PR #2 merged as `10ba8982` and deployed immutable review snapshots, two server-owned review cycles, and JT decisions bound to exact draft/snapshot hashes; 196 full tests, 67 fresh focused tests, TypeScript, and the 41-route build passed; protected routes fail closed until separate capabilities are configured. | Human-review integrity, least-privilege capability design, fail-closed deployment verification | Complete / internal proof only |
| 2026-09-14 | AI Workflow Growth OS Checkpoint 1B | Mission Control's dedicated create-only endpoint was deployed and verified not to overwrite an existing task; the dependent `jt-ops` bridge and inactive weekly-heartbeat renderer merged as `a9f3a690`; the exact post-merge `main` run passed all nine CI jobs. Production writes and heartbeat scheduling remain inactive pending separate approval. | Cross-repository agent integration, atomic human-task protection, fail-closed evidence validation, approval-gated automation | Complete / internal proof only |
| 2026-08-24 | Bounded Nightly Validation Operating System | Replaced broad night-autonomy loops with a three-phase admission, reconciliation, and consume controller; strict evidence-root, hash, privacy, verifier, score, distribution, fatal-constraint, and dedupe gates; Mission Control read-only Overnight plus Now, Paid Delivery, Career Hedge, Compounding Bets, Waiting, and Archive projections; 20 controller tests and 126 Mission Control tests pass; isolated 38-page build passes; 11:15 PM ET cron passed silent empty-queue live runs; 60-system registry records revival triggers. | Agent orchestration, verifier-gated autonomy, evidence integrity, task-admission governance, tested internal product delivery | Complete / internal proof only |
| 2026-08-22 | Evidence-Backed Job Hedge Pipeline | Reactivated weekday job research at 05:15 ET and Tue/Thu application tracking with direct-fit gates, live-posting verification, $150K floor, GPT-5.6 Sol package routing, disabled auto-builder, archived legacy definitions, passing model/volume guards, and fresh-context CONFIRMED verdict. | Agent workflow redesign, candidate-screening controls, cron governance, independent verification | Complete / internal proof only |
| 2026-08-18 | MSI / Marketsmith Nexus SOW | MSI fixed-scope Nexus engagement closed/accepted: all four SOW deliverables delivered and accepted; client technical lead independently re-verified on MSI systems and confirmed to exec team on 2026-08-17. Reusable proof is anonymized only: 500+ test cases, six quality checks, 224 mutation-tested behaviours, four spend-total query grains agreeing to the cent, full fiscal-calendar source verification, 11 defects found/fixed only as "tool working" framing, and delivery inside fixed 80 hours. Client name and internal hours-saved estimates remain permission-blocked. | Client delivery proof, analytics QA controls, mutation testing, proof-safe case-study packaging | Complete / permission-gated |
| 2026-08-10 | Mission Control Today Drawer Trust Fix | Fixed Today drawer status/priority action wiring, duplicate Inspect/Open behavior, proof-warning display, and rank-audit formatting; verified `bun test lib/mission-control` 109 pass, `bunx tsc --noEmit`, `bun run build`, and live Playwright smoke against `localhost:3000` with `PATCH /api/tasks` intercepted for Done. | Regression-driven product repair, dashboard trust, task-action wiring, proof-aware UI display | Complete |
| 2026-07-09 | Mission Control Passive Income Decision Board | Restored `/passive-income` as a visible Ship subpage and rebuilt it into a decision board over `memory/passive-income/`; verified 44 deduped ideas, 44 score rationales, 39 scorecard breakdowns, source file links, filters/search/sort, `/ship` entry point, focused Bun tests, isolated Next build, local `/api/passive-income`, local `/passive-income` HTTP 200, and desktop/mobile screenshots under `/tmp/mission-control-screens/`. | Product UX repair, decision-board design, local report parsing, regression-tested route discoverability | Complete |
| 2026-07-06 | jtsomwaru.com Homepage Positioning Broadening | Repositioned the homepage from property-first copy to broad AI operations implementation for ops-heavy teams while preserving `/property` as the focused PM referral path; updated hero, buyer segments, systems, client proof, nav order, metadata, JSON-LD, `llms.txt`, and project instructions. Verified `npm run lint`, `npm run build`, local `/` 200 with the new hero, and local `/property` still property-specific; pushed commit `c870c52`. | Consulting positioning correction, vertical wedge separation, proof-safe copy, AI-search metadata maintenance | Complete |
| 2026-07-03 | jtsomwaru.com Workflow Audit Conversion Pass | Converted the homepage to one Workflow Audit front door, added `/property` as a forwardable property-ops referral page, replaced the unverified construction hours claim with proof-safe production-status wording, updated sitemap/llms/JSON-LD surfaces, and verified `npm run lint`, `npm run build`, and local route probes for `/`, `/property`, `/sitemap.xml`, and `/llms.txt`. | Consulting conversion architecture, proof-safe copy, AI-search metadata maintenance, warm-referral page packaging | Complete |
| 2026-07-02 | Karen Vitale SoberLife-Coach Website | Finished and verified the live client site at `https://www.soberlife-coach.com/`; `soberlife-coach.com` redirects to `www`, returns HTTP 200 on Vercel, and exposes title `SoberLife-Coach | Confidential Recovery Coaching` plus description metadata for confidential recovery coaching. | Local-service website delivery, sensitive-service positioning, metadata/launch verification, client web presence packaging | Complete |
| 2026-06-18 | Glow Index Product Analysis SEO/GEO Pass | Upgraded existing `/rankings/[id]` product pages with explicit `AI skincare analysis` extraction copy, source tag `glow_seo_product_analysis_20260618`, GA4 `view_product.source_tag`, and category internal links; batched `/rankings` canonical metadata, CollectionPage/ItemList/FAQ JSON-LD, source tag `glow_seo_rankings_index_20260618`, and `llms.txt` hierarchy clarification into final commit `3c60c25`; red/green assertions passed, lint passed with two known warnings, build passed, crawler check stayed `all_clear=true`, and production live verification passed after republish. | GEO product-page optimization, acquisition instrumentation, crawl-gated app marketing execution | Complete |
| 2026-06-16 | Mission Control Slice 1.7 Health Lane | Promoted Health to `/health` with ops failures, cost pressure, stale risk, and recovery work from current signals and cost data; `/monitor` and `/costs` redirect to `/health`, `/legacy/monitor` and `/legacy/costs` remain reachable, 44 Bun tests passed, TypeScript passed, isolated Next build passed, HTTP checks passed, and mobile screenshot `/tmp/mission-control-screens/health-mobile-cockpit.png` showed the ops health cockpit. | Ops-health cockpit design, route migration, cost/risk signal grouping, reliability-focused product UX | Complete |
| 2026-06-16 | Mission Control Slice 1.6 Evidence Lane | Promoted Evidence to `/evidence` with buyer proof, system proof, content assets, and proof gaps from current proof signals; `/audit` redirects to `/evidence`, `/legacy/audit` remains reachable, 40 Bun tests passed, TypeScript passed, isolated Next build passed, HTTP checks passed, and mobile screenshot `/tmp/mission-control-screens/evidence-mobile-ledger.png` showed the proof ledger. | Proof-led operating-system UX, route migration, trust-led evidence grouping, audit-log adaptation | Complete |
| 2026-06-16 | Mission Control Slice 1.5 Machine Lane | Promoted Machine to `/machine` with cron health, agent posture, cost pressure, automation risks, and recent system work from current Mission Control signals; `/agents` redirects to `/machine`, `/legacy/agents` remains reachable, 37 Bun tests passed, TypeScript passed, isolated Next build passed, HTTP checks passed, and mobile screenshot `/tmp/mission-control-screens/machine-mobile-cockpit.png` showed the system-health cockpit. | Machine-status cockpit design, route migration, cron/agent/cost signal modeling, reliability-focused product UX | Complete |
| 2026-06-16 | Mission Control Slice 1.4 Ship Lane | Promoted Ship to `/ship` with app distribution, content queue, release gates, proof coverage, and blocked/stale counters from current Mission Control signals; `/vibe` redirects to `/ship`, `/legacy/vibe` remains reachable, 34 Bun tests passed, TypeScript passed, isolated Next build passed, HTTP checks passed, and mobile screenshot `/tmp/mission-control-screens/ship-mobile-cockpit-final.png` showed live Ship counters. | Product-shipping cockpit design, route migration, proof-aware app/content operations | Complete |
| 2026-06-16 | Mission Control Command Attention Brief | Replaced the static `Since Last Look` block on `/` with a tested Command Brief that surfaces top action, latest proof, urgent JT count, revenue pressure, and risk count from current signals; 31 Bun tests passed, TypeScript passed, isolated Next build passed, `/` returned 200 with `Command Brief`, and mobile screenshot `/tmp/mission-control-screens/command-mobile-brief.png` was captured. | Operating-cockpit UX, tested attention summarization, proof/revenue/risk signal compression | Complete |
| 2026-06-16 | Mission Control Command Cockpit Slice 1 | Rebuilt Mission Control's home surface into a 7-lane cockpit with ranked JT decision queue, revenue cockpit, Eve-handling column, risk/drift column, Work task-router lane, signal adapters, score tests, inspection drawer, and state blocks; 11/11 Bun tests passed, TypeScript passed, isolated Next build passed, local `/` and `/work` returned 200, and desktop/mobile screenshots were captured. | Agent operating-system UX, tested decision ranking, proof-backed internal ops design, current-data product execution | Complete |
| 2026-06-16 | Mission Control Slice 1.1 Mobile Shell + Revenue Lane | Centered and safe-area-hardened the mobile bottom nav, replaced `/consulting` with a live cash-path cockpit from North Star/pipeline/task data, preserved the old consulting strategy page at `/legacy/consulting`, added explicit Todo/Doing/Done controls, priority-first sorting, visual priority treatment, and an inspection/action drawer on mobile `/work`, and verified 28 Bun tests, TypeScript, isolated Next build, HTTP checks, task PATCH API create/update/archive/delete, and mobile/desktop screenshots. | Revenue-ops cockpit design, mobile product polish, deterministic local-data parsing, cash-path prioritization | Complete |
| 2026-06-13 | jtsomwaru.com Phase 3 Site Hygiene | Added Consulting/Product blog categories, diagram-based OG/Twitter images for `/work/local-first-family-office-automation`, `/work/construction-tracker`, and `/work/streeteasy-scraper`, Calendly `generate_lead` tracking, `docs/EMAIL_ALIAS_HANDOFF.md`, and `scripts/verify-phase3-site-hygiene.mjs`; pushed commit `9a7fad2`; Vercel production status completed; production verifier passed against `https://jtsomwaru.com` with 44 pages, 7 external links, 3 OG images, and 0 failures. | Site hygiene, AI-search/social-share metadata, conversion tracking instrumentation, proof-safe launch readiness | Complete |
| 2026-06-13 | jtsomwaru.com Phase 2 Offer Pages | Added live `/services/coi-tracking-property-management` and `/services/rent-delinquency-automation` pages with Service/FAQ JSON-LD, sitemap/llms exposure, homepage service routing, and a held silent-failure teardown draft; pushed commit `9a7fad2`; production checks returned 200 for both routes and `/sitemap.xml`; full production verifier passed with 0 failures. | Consulting offer packaging, AI-search service-page implementation, property-ops workflow positioning | Complete |
| 2026-06-12 | jtsomwaru.com Phase 1 Fable Copy Overhaul | Built preview-only Fable-aligned copy changes for `jtsomwaru.com`: updated Consulting Pipeline Agent, Agent Operations, Content Intelligence, CTA/title language, hero roles-link removal, and case-study footer retainer wording; captured before/after screenshots in `reports/screenshots/2026-06-12-phase1/`; `npm run lint` and `NEXT_DIST_DIR=.next-build npm run build` passed with 51/51 static pages. Vercel preview deploy is blocked by missing credentials. | Consulting-positioning cleanup, proof-safe copy editing, preview evidence discipline | Preview complete |
...
## Global punctuation and structure rule — 2026-08-24

- Do not use a colon to introduce a list, sequence, explanation, or rhetorical reveal in JT-facing content. The `setup: item, item, item` shape reads as AI-generated.
- Rewrite with a full sentence, separate sentences, bullets without a colon-led setup, or natural prose.
- Applicant-visible cover letters use zero colons unless JT explicitly requests otherwise.
