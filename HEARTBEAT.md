...
5. Read `memory/north-star.md`, `memory/pipeline.jsonl`, and `memory/send-queue.md`. SEND QUEUE ordering is pipeline-advancing first, content second, everything else third. Max 3 finished artifacts; each needs a direct link/path and one-word reply keyword.
6. Send punchy bullets with one action. Sections: SEND QUEUE (max 3 finished artifacts), priorities, news, niche intel, jobs, costs, workout, vibe queue, Nash X+Reddit excerpts/links when generated, Dynasty X, one action. As of 2026-06-11, Nash full text appears only when it is selected as a capped Send Queue item.
