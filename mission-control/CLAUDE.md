...
Outreach review admission uses `POST /api/tasks/outreach-review`, never generic create-only task admission. It requires `X-Outreach-Review-Capability` matching `OUTREACH_REVIEW_CAPABILITY` and atomically stamps the task's server-owned `outreachReview` marker; candidate/draft fields without that marker never expose decision controls. A marked review task is a fully immutable snapshot: every generic update, status, pipeline-stage, upsert, delete, auto-archive, and backfill path must reject or skip it. Only the specialized decision mutation may append its first decision. Outreach decisions use `/api/tasks/outreach-decision`: `POST` requires the Tailscale Serve `Tailscale-User-Login` header to equal `OUTREACH_DECISION_JT_LOGIN`, then uses the distinct `OUTREACH_DECISION_CAPABILITY` to bind the first immutable JT `approve`/`reject` decision to `taskId + candidateId + draftSha256`; a reversal or changed draft requires a new versioned task. `GET ?candidateId=<id>&draftSha256=<64-lowercase-hex>` is the read-only exact lookup for pre-send checks and fails closed as `absent` or `rejected`; an archived card is always `absent`. Configure nonblank, unequal review and decision capability secrets in their Next and Convex environments, never in source or logs; every protected boundary returns 503 when either is missing/blank or they are equal. Capability comparison uses Web Crypto HMAC-SHA-256 verification, never a JavaScript equality loop. Protected HTTP routes expose only enumerated safe validation/conflict messages; unknown Convex or dependency failures return a generic 500 without downstream details. An automated review-admission caller must never receive the decision capability. Generic task POST/PATCH/create-only cannot write review or decision authority fields. This authentication assumes the existing `next dev -H 127.0.0.1` binding behind Tailscale Serve; do not expose the backend directly.
