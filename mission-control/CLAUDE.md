...
Review-authority runtime configuration adds distinct `review-authority-write` and `review-authority-read` Keychain kinds without weakening the mandatory review/decision pair. The authority pair is optional only as a complete pair: when both items are absent, existing services start, Next omits all authority variables, Convex sync explicitly deletes all three authority variables, and `/api/tasks/outreach-review-authority` alone returns 503; a partial pair, present-but-blank value, or any collision among all four capabilities fails startup and Convex sync closed. Explicit `install` rotation generates all four pairwise-distinct values. A complete pair is injected identically into Next and Convex with fixed server verifier role `OUTREACH_REVIEW_AUTHORITY_VERIFIER_ACTOR_ID=openclaw:review-verifier-v1`. The runtime silently probes the ignored helper's versioned four-capability protocol and rebuilds a stale binary before any Keychain operation. Never print or pass capability values in arguments. Operational contract: `docs/operations/outreach-capabilities.md`.
