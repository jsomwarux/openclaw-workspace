...
Deployment status (2026-09-15): PR #2 merged as `10ba8982`, PR #3 merged as `bf02b25f`, and the outreach contract plus reproducible runtime package are live on local/tailnet Mission Control. Distinct review/decision capabilities live in macOS Keychain and are injected by `scripts/outreach-runtime-secrets.mjs`; the same helper synchronizes only those two values to local Convex, derives JT identity from Tailscale, and rebuilds the ignored Keychain binary from checked-in Swift source when absent. Live proofs confirmed two-cycle admission, third-cycle rejection, immutable JT rejection, wrong-capability denial, runtime regeneration, and universal-card compatibility after restart. Reproducible configuration and recovery steps live in `docs/outreach-runtime-configuration.md`. PR #39, production writes, and heartbeat scheduling remain inactive.
