...
Outreach review admission uses `POST /api/tasks/outreach-review`, never generic create-only task admission. It requires the server capability header and atomically stamps the task's server-owned `outreachReview` marker; candidate/draft fields without that marker never expose decision controls. Outreach decisions use `/api/tasks/outreach-decision`: `POST` requires the Tailscale Serve `Tailscale-User-Login` header to equal `OUTREACH_DECISION_JT_LOGIN`, then binds the first immutable JT `approve`/`reject` decision to `taskId + candidateId + draftSha256`; a reversal or changed draft requires a new versioned task. `GET ?candidateId=<id>&draftSha256=<64-lowercase-hex>` is the read-only exact lookup for pre-send checks and fails closed as `absent` or `rejected`. Both Convex write mutations require `OUTREACH_DECISION_CAPABILITY`; configure the same secret in the Next and Convex environments, never in source or logs. Generic task POST/PATCH/create-only cannot write review or decision authority fields. This authentication assumes the existing `next dev -H 127.0.0.1` binding behind Tailscale Serve; do not expose the backend directly.
