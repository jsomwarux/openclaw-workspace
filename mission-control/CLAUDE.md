...
Universal task cards use seven real fields: `title`, `whyItMatters`, `exactSteps`, optional `pasteReadyPrompt`, optional `pasteDestination`, `doneState`, and `feedback`. Legacy tasks may omit the additive fields. The inspection drawer renders exact steps in order, presents the prompt as copy-ready text beside its paste/use destination, and provides the feedback box. Feedback history is append-only: generic create/update/upsert paths must reject `feedback`; only `tasks.appendFeedback` through `PATCH /api/tasks` with `action: "append-feedback"` may add a server-timestamped JT/Eve entry. Appending feedback to an outreach review card does not alter its immutable outreach snapshot or decision binding.

Outreach review admission uses `POST /api/tasks/outreach-review`, never generic create-only task admission. It requires `X-Outreach-Review-Capability` matching `OUTREACH_REVIEW_CAPABILITY` and atomically stores the complete immutable review snapshot: candidate/cohort/draft identity, subject, body, verbatim verifier report, review/verifier actor IDs, and evidence/policy/gate/draft/verifier Git bindings. Mission Control computes the versioned canonical snapshot hash, assigns review cycle 1 or 2, and derives task display fields; exact retries are idempotent and a third distinct cycle fails closed. `GET` on the same route returns only the authoritative count/latest state. Generic tasks never expose decision controls, and every generic update, status, pipeline-stage, upsert, delete, auto-archive, and backfill path must reject or skip server review snapshots. Only the specialized decision mutation may append the first immutable decision. Outreach decisions use `/api/tasks/outreach-decision`: `POST` requires the Tailscale Serve `Tailscale-User-Login` header to equal `OUTREACH_DECISION_JT_LOGIN`, then uses the distinct `OUTREACH_DECISION_CAPABILITY` to bind JT's immutable `approve`/`reject` decision to `taskId + candidateId + draftSha256 + snapshotSha256`; a reversal or changed snapshot requires a new versioned task. `GET ?candidateId=<id>&draftSha256=<64-lowercase-hex>&snapshotSha256=<64-lowercase-hex>` is the read-only exact lookup for pre-send checks and fails closed as `absent` or `rejected`; an archived card is always `absent`. Configure nonblank, unequal review and decision capabilities in their Next and Convex environments, never in source or logs; every protected boundary returns 503 when either is missing/blank or they are equal. Capability comparison uses Web Crypto HMAC-SHA-256 verification. Protected HTTP routes expose only enumerated safe validation/conflict messages; unknown Convex or dependency failures return a generic 500 without downstream details. An automated review-admission caller must never receive the decision capability. Full integration contract: `docs/mission-control-outreach-review-contract.md`. This authentication assumes the existing `next dev -H 127.0.0.1` binding behind Tailscale Serve; do not expose the backend directly.

Deployment status (2026-09-15): PR #2 merged as `10ba8982` and the contract is live on local/tailnet Mission Control. Distinct review/decision capabilities live in macOS Keychain and are injected by `scripts/outreach-runtime-secrets.mjs`; the same helper synchronizes only those two values to local Convex and derives JT identity from Tailscale. A live synthetic no-send proof confirmed two-cycle admission, third-cycle rejection, immutable JT rejection, and wrong-capability denial. Reproducible configuration and recovery steps live in `docs/outreach-runtime-configuration.md`. PR #39, production writes, and heartbeat scheduling remain inactive.

Review-authority runtime configuration adds distinct write/read capabilities without weakening the mandatory review/decision pair. Explicit `install` rotation generates four pairwise-distinct values and atomically stores one versioned Keychain capability-set item; `read-set` returns that complete set in one captured response for exact JS validation. When the set is absent, the only allowed legacy fallback reads existing review and decision items under the same owner-only lock and treats authority as absent. Next then omits all authority variables, Convex sync explicitly deletes all three authority variables, and `/api/tasks/outreach-review-authority` alone returns 503. Partial/blank/colliding sets fail closed. A complete set maps fixed verifier role `OUTREACH_REVIEW_AUTHORITY_VERIFIER_ACTOR_ID=openclaw:review-verifier-v1`. `.runtime/outreach-capability.lock` covers helper validation plus each read/install/sync; helper reuse also requires a v3 probe and a source-derived owner-only stamp. Rebuilds compile privately, validate, and atomically rename under that lock. Never log or pass capability values in arguments. Operational contract: `docs/operations/outreach-capabilities.md`.
