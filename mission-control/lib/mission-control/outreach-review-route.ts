...
import { assertServerCapability, OutreachAuthError } from "./outreach-auth";
...
export function createOutreachReviewPostHandler(dependencies: Dependencies) {
  return async function POST(req: Request) {
    try {
      ...
      assertServerCapability(callerCapability, dependencies.serverCapability);
      ...
      const result = await dependencies.admit({ ...input, capability: dependencies.serverCapability });
