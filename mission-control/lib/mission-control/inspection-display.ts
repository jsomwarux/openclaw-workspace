...
export function operatingSystemDetails(signal: Signal): Array<[string, string]> {
  const values: Array<[string, string | number | undefined]> = [
    ...
    ["Exact steps", signal.exactSteps?.map((step, index) => `${index + 1}. ${step}`).join("\n")],
    ["Paste-ready prompt", signal.pasteReadyPrompt],
    ["Paste/use location", signal.pasteDestination],
    ["Done condition", signal.doneState],
