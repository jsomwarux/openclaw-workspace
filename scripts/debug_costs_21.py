import json
try:
    d = json.load(open('/Users/jtsomwaru/.openclaw/workspace/memory/costs/2026-04-21.json'))
    ...
    for k, v in sess[:15]:
        ...
        print(f"${v.get('cost_usd', 0):>6.2f} | In: {v.get('input_tokens', 0):>7,} | Out: {v.get('output_tokens', 0):>5,} | {model[:25]:<25} | {name}")
