...
HASH_PATH = SNAPSHOT_DIR / ".jobs-json.sha256"
...
def main() -> int:
    ...
    jobs_hash = hashlib.sha256(json.dumps(parsed, sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode("utf-8")).hexdigest()
    old_hash = HASH_PATH.read_text(encoding="utf-8").strip() if HASH_PATH.exists() else ""
    if old_hash == jobs_hash:
        print(f"CRON_SNAPSHOT_NO_HASH_CHANGE {HASH_PATH.relative_to(WORKSPACE)}")
        return 0

    stamp = datetime.now().strftime("%Y-%m-%d")
    target = SNAPSHOT_DIR / f"jobs-{stamp}.json"
    target.write_text(json.dumps(parsed, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    HASH_PATH.write_text(jobs_hash + "\n", encoding="utf-8")

    rel = target.relative_to(WORKSPACE)
    hash_rel = HASH_PATH.relative_to(WORKSPACE)
    add = run(["git", "add", str(rel), str(hash_rel)])
    if add.returncode != 0:
        print(add.stderr.strip() or add.stdout.strip(), file=sys.stderr)
        return add.returncode

    diff = run(["git", "diff", "--cached", "--quiet", "--", str(rel), str(hash_rel)])
    ...
    commit = run(["git", "commit", "-m", f"chore: snapshot cron jobs {stamp}", "--", str(rel), str(hash_rel)])
