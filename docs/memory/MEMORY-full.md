...
- 2026-08-18 strategy clarification: for the next 90 days, cash collected is the main outcome. JT is willing to be property-first publicly during that window while backend research compares adjacent niches. Preferred end-state business is a managed AI operations service plus a partner-business studio that installs controlled AI operating systems.
...
- 2026-08-10 JT sync correction supersedes the stale parts of the 2026-07-16 alignment: split Altmark FTE + NewCo from Ron/BDT; treat Altmark FTE + NewCo as active negotiation with Yair and Adai, with employment and NewCo as separate contracts and JT's existing systems/toolkit/playbook remaining personal IP. Ron/BDT Diamond District is separate watch-only and Ron has not been briefed on the real-estate developments. Karen/SoberLife is paid/closed-won but NOT referral-eligible until SEO expectation reset is in writing and handoff items close. Aya is two clients/lanes: Gil construction dashboard is complete/paid/referral-eligible; Maiky is a separate live opportunity on hold after cost review. DHCR proposal went to Matt and routes as delivery-track; do not chase deposit inside the compensation negotiation thread. Guyana is capped exploration, not killed: Paul James / The SaPaJ Group warm intro via Rosh is in flight below the top four priorities. Mission Control 251 active/0 overdue is stale-backlog signal, not health; quote counts only after triage. Collected dollar figures must cite the payments ledger; `memory/pipeline.jsonl` is forecast-only.
