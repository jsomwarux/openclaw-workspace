...
> Full detail: `docs/memory/MEMORY-full.md`.
> Compact file: current facts + pointers only.
> Growth rule: new durable entries are one line here with detail appended to `docs/memory/MEMORY-full.md`; warn JT when this index passes 7,000 chars.
> Bootstrap figure covers exactly `AGENTS.md`, `MEMORY.md`, `TOOLS.md`, and `HEARTBEAT.md`.
...
## Active Clients
- Altmark is the top paid/proof lane; insurance final 50% and rent delinquency initial 50% are collected, but rent is now blocked on Yair's source/export inputs before remaining payment/workflows unlock.
- Aya remains anchor proof history: dashboard done; StreetEasy is cancelled, co-living is dead currently, acquisitions is stalled.
...
- Phase 2A/Finding 1: audio autodetect enabled, LCM reset; Moonshot fallback blocked by account billing (`disabled:billing`), pending JT recharge/reactivation before smoke/apply.
...
- Phase 7: 53 enabled / 70 total; 9:45PM Nightly Leverage + 3:20AM Overnight Autonomy disabled, replaced by 11PM `Night Autonomy Agent` (`f146d8b8`).
...
- Morning Brief opens with capped Send Queue; Evening Digest runs 7PM ET; `critical-files-integrity` now snapshots `jobs.json` daily via `scripts/cron_snapshot.py`.
...
- Never fabricate; verify claims with tool/script evidence for outreach, URLs, deployments, Drive links, and task closure.
- JT corrections require immediate Mistakes Log/rule updates with regression checks.
- LCM recall guidance: compacted history is cue only; use `lcm_grep`, `lcm_describe`, or `lcm_expand_query` before exact claims.
