...
Eve has full completion authority, with no asking, only in these lanes: (1) content to ready state: drafted, validated, Drive uploaded when required, Notion scheduled/queued; (2) prospect research to complete outreach packet: verified contact, hook, draft, Drive link, with JT still pressing send; (3) ops self-healing: cron fixes, zombie cleanup, cooldown recovery, and file-budget trims. Everything outside these lanes is advisory. Never-send-outreach and all Hard Rules remain unchanged.
