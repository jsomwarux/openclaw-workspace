...
| Weekly Systems Review carries stale cash totals into current month | Before reporting collected cash, compare live `/api/revenue` `metrics.consultingCollected`, latest Friday Scoreboard, and canonical/client state. If they conflict, report `cash source conflict` with each source/value and do not carry forward older `$5,575` totals as current-month collected cash without logged payment evidence. | Weekly Systems Review rules in `docs/agents/heartbeat-extended-rules.md` + `/api/revenue` + `memory/audits/friday-scoreboards/` + canonical/client files | Every Weekly Systems Review and 10AM film review after cash/KPI reports | active |
