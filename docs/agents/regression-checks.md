...
- **Fail condition:** Scout or Strategist cron reads broad history, does live research, burns 100K+ tokens, relies on an LLM final confirmation before writing artifacts, labels stale-but-readable inputs invalid, writes `INCOMPLETE` when a compact degraded artifact could be produced, uses pseudo-command syntax for searches/checks, accepts a stale delivery marker after overwriting a strategist report, or leaves `pre-scout`, `pre-strategist`, or `post-strategist` failing for the current date.
