...
| Health check-in delivers but pending state is not written | After the 9PM Health Check-in cron, verify latest run delivery fields and `health/pending-checkin.json` date. If Telegram delivered but the file is stale because the cron lacked filesystem tools, repair it locally from the run timestamp and keep the cron payload's `toolsAllow` including `read`, `write`, `message`, and `exec`. | Health Check-in cron payload + `health/pending-checkin.json` + HEARTBEAT cron health checks | 10AM film review after health delivery gaps; heartbeat on health check-in errors | active |
...
- **Trigger:** Any passive-income Scout, Strategist, or delivery-guard failure, or any edit to `agents/passive-income-scout/AGENT.md`, the Scout cron payload, `scripts/passive_income_handoff_check.py`, or `scripts/passive_income_strategist_delivery_guard.py`.
- **Check:** Signal fetch should refresh hard sources when possible, but stale inputs become `degraded=true` warnings rather than invalid handoffs. Scout prompts must run `passive_income_handoff_check.py --mode pre-scout` before research, then remain bounded: current weekly signal files, `state.json`, last 4 scout reports, last 4 strategist reports, at most 6 live searches, and 4-6 raw ideas. Live X/web search is optional and must not be on the artifact-critical path. If a live-search command is malformed, unavailable, or low-signal, Scout must write a same-day `DEGRADED RECOVERY RUN` artifact from local signals and verify `pre-strategist` before the Strategist window. Cron and agent prompts must use real executable shell commands with full paths, never pseudo tool chains like `run source -> run python3 ... (agent)`.
- **Fail condition:** Scout cron reads all historical passive-income reports, burns 100K+ tokens without writing the same-day scout artifact, labels stale-but-readable inputs invalid, writes `INCOMPLETE` when a compact degraded report could be produced, stops after a malformed optional live-search command instead of writing a degraded artifact, uses pseudo-command syntax for searches/checks, or delivery guard labels a delivered degraded/block report as a silent failure.
- **Command:** `python3 -m py_compile scripts/fetch-signals.py scripts/passive_income_handoff_check.py scripts/passive_income_strategist_delivery_guard.py`; after rerun, `python3 scripts/passive_income_handoff_check.py --mode pre-scout`, `python3 scripts/passive_income_handoff_check.py --mode pre-strategist`, and when the strategist has run, `python3 scripts/passive_income_handoff_check.py --mode post-strategist` must return `PASSIVE_INCOME_HANDOFF_OK`; `rg -n "Critical-path rule|Command safety" agents/passive-income-scout/AGENT.md` must show both guardrails.
- **Owner surface:** `scripts/fetch-signals.py`, `agents/passive-income-scout/AGENT.md`, `agents/passive-income-strategist/AGENT.md`, passive-income-scout cron payload, `scripts/passive_income_handoff_check.py`, `scripts/passive_income_strategist_delivery_guard.py`, passive-income strategist delivery guard cron.
