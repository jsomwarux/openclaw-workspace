...
| Cron prompts issue pseudo-commands instead of real shell commands | Cron payloads must spell executable commands with full workspace paths when asking agents to search/read/check files. Do not use pseudo tool-call text like `search "..." in FILE (agent)`, chained prose such as `create folder -> print text -> run script (agent)`, or truncated `jq ... (agent)` strings; use `rg`, `sed`, `jq`, or a script file, and make expected no-match searches nonfatal with `|| true`. Do not pipe `curl` into an inline Python heredoc because the heredoc consumes stdin; use `python3 -c`, a script file, `jq`, or Python `urllib.request` instead. | Cron payloads + agent prompts + Nightly Leverage command-safety hardening + heartbeat command snippets | Every cron-health fix and 10AM film review | active |
