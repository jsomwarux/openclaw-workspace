...
| Daily Send Sheet surfaces stale routing after same-day source reconciliation | When `memory/send-queue.md`, `memory/job-state/daily-send-sheet.md`, or a client action file is updated after the scheduled Daily Send Sheet, the next 10AM film review must verify SENDS DUE against the current send queue and job-state files. Superseded proposal packets, benched outbound items, and distinct client deposits must not remain surfaced as active send actions after reconciliation. | Daily Send Sheet output + `memory/send-queue.md` + `memory/job-state/daily-send-sheet.md` + client action files | Every 10AM film review after same-day send-sheet reconciliation | active |
