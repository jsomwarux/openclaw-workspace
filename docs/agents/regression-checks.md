...
| Writable skill collection exceeds the review limit | Before expanding any live `SKILL.md`, total writable skill bytes and compare with the 240,000-byte collection-review cap; keep active instructions concise and route detail into bundled references through Skill Workshop. If Workshop does not own a legacy path, record the blocked migration for Weekly Systems Review and do not bypass ownership by editing the skill directly. | Skill Workshop + HEARTBEAT cron health + Weekly Systems Review | Every skill update and after any collection-review size error | active |
...
| Backup local snapshot succeeds but repo push fails repeatedly | If `reports/backup-alerts/YYYY-MM-DD.md` contains a nonfatal GitHub push failure, the next 10AM film review must inspect `~/.openclaw/backups/backup.log` for the exact repo and cause, compare with the previous day's backup alert, and record recurrence when the same repo fails on consecutive days. After a repair, require the next natural backup to show an explicit successful push for the affected repo and no same-day backup alert before clearing the failure narrative. Do not run `git pull`, merge, force-push, or mutate the external repo from a heartbeat; create/route a follow-up only when the same repo failure repeats or blocks recovery. | `scripts/backup.sh` + `reports/backup-alerts/` + `~/.openclaw/backups/backup.log` + HEARTBEAT 10AM film review | Every 10AM film review after a backup alert or repair | active |
| Heartbeat output relies on unresolved source-channel alias | When a heartbeat or reminder needs to deliver a packet/output to JT, do not rely on the default source-channel alias if it has recently resolved as `@heartbeat`; use numeric Telegram `6608544825` as the first delivery path for Daily Delta/packet outputs, then log the message id or delivery failure. Delivery calls must set an explicit numeric destination field (`chatId`/`to`/equivalent) instead of relying on the current source conversation. If a default source-channel paste is attempted and resolves as `@heartbeat`, treat that as recurrence evidence even when the numeric retry succeeds. | HEARTBEAT 22:00 packet handling + message delivery calls + daily notes | 10PM Daily Delta handling and 10AM film review after delivery errors | active |
| Manual cron repair leaves scheduler state stuck | After any manual cron run, cron prompt rewrite, gateway restart, or stuck-`runningAtMs` repair, verify `openclaw cron list --json` for that job shows `enabled:true`, `consecutiveErrors:0`, a future `nextRunAtMs`, and no lingering `runningAtMs` before closing the repair. | Cron repair runbook + affected cron payload/state + this file | Every manual cron repair or gateway restart touching scheduler state | active |
| Stale red cron rows trigger repeat prompt patches before the repaired natural run | When the latest red run predates a prompt hardening or repair, compare latest run `runAtMs` with the repair date and `nextRunAtMs`; do not patch again or blind-rerun solely to clear metadata before the next natural run unless fresh run evidence shows a new blocker. Record the expected natural verification window instead. | HEARTBEAT cron health checks + affected cron payloads + daily notes | Every cron-health fix and 10AM film review | active |
| Auth cooldown outage leaves stale cron red rows after gateway recovery | After a broad `No available auth profile for openai-codex` outage, verify `openclaw gateway status`, `openclaw models status`, and fresh `openclaw cron list --json`; treat old red metadata as stale only while natural reruns are clearing, and confirm the enabled red count reaches 0 before closing the runtime-drift loop. | HEARTBEAT cron health, OpenClaw gateway/model status, runtime-drift Mission Control task | Every auth/cooldown recovery and 10AM film review until clear | active |
| User-facing reminder cron fails on Codex subscription usage limit | When a user-facing reminder/packet cron fails with `errorReason=rate_limit` or a Codex subscription usage-limit message, verify latest run history, delivery status, failure-alert delivery status, configured model/provider, and reset time before alerting. Do not rerun, switch provider/model, or edit cron payload from heartbeat unless explicitly approved; route as a user decision or supervised ops fix and suppress duplicate notifications until the reset/provider state changes. | Nightly Claude Delta Packet Reminder cron + HEARTBEAT cron health + daily notes | Every heartbeat while the cron is unhealthy; 10AM film review after first occurrence | active |
| Weekly user-facing cron remains red after the stated subscription reset but before its next natural run | Verify the reset timestamp, the cron's exact `nextRunAtIso`, and latest run/delivery fields. Label the state `awaiting scheduled validation` until that natural run occurs; do not rerun, reroute, or claim recovery early. Immediately after the scheduled run, require `status=ok` and `deliveryStatus=delivered`; if either fails, notify once with the new evidence and route the blocker. | Friday Scoreboard cron + HEARTBEAT cron health + daily notes | First 10AM film review after reset, then first heartbeat after the next natural run | active |
| Recovered user-facing reminder keeps stale failure narrative | After a user-facing reminder/packet cron recovers from a prior red/rate-limit state, the next heartbeat film review must verify latest run `status`, `deliveryStatus`, provider/model, and `nextRunAtIso` before carrying forward any blocker language. If the latest run delivered, log the recovery and suppress stale-failure notifications while preserving the original regression row for future red runs. | Nightly Claude Delta Packet Reminder cron + HEARTBEAT 10AM film review + daily notes | 10AM film review after a reminder cron recovers | active |
| Cron repair uses provider/runtime spelling instead of configured model key | After any cron model-route repair, run `python3 scripts/model_routing_guard.py` and inspect the edited job payload with `openclaw cron list --json`; payload models must use configured keys such as `openai/gpt-5.5`, not runtime/provider pairs such as `openai-codex/gpt-5.5`. | `scripts/model_routing_guard.py` + affected cron payloads + HEARTBEAT cost/model checks | Every cron model-route repair and 10AM film review | active |
...
| System-owned heartbeat schedule drifts from the authoritative four daily gates | Verify the live `heartbeat:main` automation uses `0 10,14,18,22 * * *` in `America/New_York`. An hourly `everyMs` schedule is a failure because it creates duplicate wakes and logs. Attempt schedule repair only through the automations interface; if the scheduler rejects the edit as system-owned, record the exact blocker in the existing runtime-drift Mission Control task and do not bypass ownership through raw config edits. | `heartbeat:main` automation + HEARTBEAT.md + runtime-drift Mission Control task | Every 10AM film review and after monitor provisioning changes | active |
| Missing prior-day note causes film-review history to be inferred | At the 10AM film review, require `memory/YYYY-MM-DD.md` for yesterday. If absent, log the evidence gap, use only dated first-party artifacts for any limited review, and never reconstruct activity from session history. Verify the current-day note records the gap and guardrail. | `docs/agents/heartbeat-extended-rules.md` + daily notes + this file | Every 10AM film review | active |
| Due-soon count changes solely because the clock crosses the window boundary | When a high-priority task enters the 48-hour window without a task mutation, identify the exact task and due time from live Mission Control, label it a time-window transition, notify once, and suppress repeats until task state or severity changes. Do not describe the audit itself as changing when `changes=0`. | Mission Control `/api/tasks` due-window query + heartbeat daily note | Every heartbeat reporting a newly changed due-soon count | active |
| July directive cron registry drifts after migration | Compare `openclaw cron list --json` against `directives/00-README.md`: exactly 12 approved recurring jobs enabled, no refused-scope jobs enabled, and approved schedules match the directive pack, especially Pending Task Processor `30 10,18 * * *` and prospect-discovery `0 23 * * 0`. | `directives/00-README.md` + OpenClaw live cron registry + HEARTBEAT 10AM checks | Every 10AM film review and after any cron registry edit | active |
| Daily Send Sheet flags a client gate without a send-ready artifact | If `GATES <=48H` or `SENDS DUE` says no finished artifact exists for an active client decision, the 10AM film review must create or point to one concrete draft/control artifact, then log it in the relevant client folder or owner surface. | Daily Send Sheet output + client folders under `memory/clients/` + HEARTBEAT 10AM film review | Every 10AM film review after a Daily Send Sheet gate/artifact gap | active |
| Mission Control client task keeps stale pricing after a commercial pivot | When a client decision log or send packet supersedes a prior rate/scope frame, update the active Mission Control task title/first action to the current send-ready path instead of letting stale hourly anchors remain in the high-priority layer. | Mission Control API + `memory/clients/*/decision-log.md` + send packet path | Every 10AM film review after a Daily Send Sheet client send or pricing gate | active |
| Daily Send Sheet surfaces stale routing after same-day source reconciliation | When `memory/send-queue.md`, `memory/job-state/daily-send-sheet.md`, `memory/pipeline.jsonl`, or a client action file is updated after the scheduled Daily Send Sheet, the next 10AM film review must verify SENDS DUE against the current send queue, pipeline, payment-cleared state, and job-state files. Superseded proposal packets, benched outbound items, distinct client deposits, and paid/cleared invoices must not remain surfaced as active send actions after reconciliation. | Daily Send Sheet output + `memory/send-queue.md` + `memory/pipeline.jsonl` + `memory/job-state/daily-send-sheet.md` + client action files + payment-cleared state | Every 10AM film review after same-day send-sheet reconciliation | active |
| Weekly Systems Review carries stale cash totals into current month | Before reporting collected cash, compare live `/api/revenue` `metrics.consultingCollected`, latest Friday Scoreboard, and canonical/client state. `scripts/north_star_pipeline.py summary --json` must source collected cash and weighted forecast from the live Mission Control revenue metrics; verify with `PYTHONPATH=scripts python3 -m unittest scripts/test_north_star_pipeline.py`. If evidence sources conflict, report `cash source conflict` with each source/value and do not carry forward older `$5,575` totals as current-month collected cash without logged payment evidence. | Weekly Systems Review rules in `docs/agents/heartbeat-extended-rules.md` + `/api/revenue` + `scripts/north_star_pipeline.py` + `scripts/test_north_star_pipeline.py` + `memory/audits/friday-scoreboards/` + canonical/client files | Every Weekly Systems Review and 10AM film review after cash/KPI reports | active |
| Digest entries surface routine FYI instead of an action | Any cron that appends to `memory/digest-queue.md` must name the concrete same-day JT action, decision, or verified critical finding; routine hypotheses, trend translations, and saved-report links must remain file-only with `Send Telegram: no`. | Evening Digest prompt + source cron delivery rules, especially `agents/reelfarm-intel/daily-prompt.md` | Every digest-producing cron patch + 10AM film review after a digest correction | active |
...
| Health check-in delivers but pending state is not written | After the 9PM Health Check-in cron, verify latest run delivery fields and `health/pending-checkin.json` date. The cron now uses `scripts/health_checkin_cron.py` as the deterministic prepare/mark-sent helper; if pending state is stale, run the helper locally, inspect the run summary, and repair only from verified Telegram delivery evidence. | Health Check-in cron payload + `scripts/health_checkin_cron.py` + `health/pending-checkin.json` + HEARTBEAT cron health checks | 10AM film review after health delivery gaps; heartbeat on health check-in errors; `python3 -m unittest scripts/test_health_checkin_cron.py` | active |
| Health check-in cron summary echoes instructions instead of helper result | After the 9PM Health Check-in cron, do not accept a run summary that is only the cron instruction payload as proof of prompt/send/skip behavior. Verify the deterministic helper result is one of `HEALTH_CHECKIN_SENT`, `SKIPPED_DUPLICATE_HEALTH_CHECKIN`, or `HEALTH_CHECKIN_FAILED`; if cron metadata lacks that result, run `python3 scripts/health_checkin_cron.py --date YYYY-MM-DD` in duplicate-safe mode and record the concrete status before closing the check. | Health Check-in cron payload + `scripts/health_checkin_cron.py` + cron run summaries + `health/pending-checkin.json` | 10AM film review after health delivery gaps; 22:00 heartbeat health verification | active |
| Health check-in fallback summary masks missing real prompt | Treat fallback delivery summaries as failure evidence, not successful check-ins. After the 9PM Health Check-in cron, verify that the intended Evening Check-in prompt was sent to the numeric Telegram chat, not only that a fallback/failure summary was delivered; if the normal prompt attempted an unresolved alias such as `@heartbeat`, resend once to the numeric chat and update `health/pending-checkin.json`. If the same fallback-mask pattern repeats on a later date, log it as recurrence evidence in the next 10AM film review instead of creating a duplicate regression row. | Health Check-in cron payload + Telegram delivery resolution + `health/pending-checkin.json` | 10AM film review after health delivery gaps; heartbeat on health check-in errors | active |
...
| Cron prompts issue pseudo-commands instead of real shell commands | Cron payloads must spell executable commands with full workspace paths when asking agents to search/read/check files. Do not use pseudo tool-call text like `run COMMAND (agent)`, `search "..." in FILE (agent)`, chained prose such as `create folder -> print text -> run script (agent)`, or truncated `jq ... (agent)` strings; use `rg`, `sed`, `jq`, or a script file, and make expected no-match searches nonfatal with `|| true`. Do not pipe `curl` into an inline Python heredoc because the heredoc consumes stdin; use `python3 -c`, a script file, `jq`, or Python `urllib.request` instead. | Cron payloads + agent prompts + Nightly Leverage command-safety hardening + heartbeat command snippets | Every cron-health fix and 10AM film review | active |
| Weekly Systems Review autoresearch checks fail after useful work because file-list/search is expressed as a pseudo agent chain | Autoresearch checklist verification must use executable shell commands such as `rg --files agents/autoresearch/checklists | rg -e "slug-a|slug-b" || true`, `test -f`, or `rg -n ... || true`; expected no-match results must be interpreted, not treated as failed runs. | Weekly Systems Review cron payload + `agents/autoresearch/checklists/` + this file | Every Weekly Systems Review and cron-health fix touching autoresearch | active |
...
| Job Market Daily stops after useful work without final confirmation | The cron payload must begin with a final-response checkpoint and accept only `JOB_MARKET_DAILY_OK`, `NO_QUALIFYING_ROLES`, or `MARKET_INTEL_ONLY` after `daily-brief.md` and required tracker/matrix artifacts are written or explicitly verified unchanged. If the run ends with "Codex stopped before confirming", inspect artifact freshness before retrying and patch the checkpoint before the next natural run. | `eve-job-market-daily-005` cron payload + `/Users/jtsomwaru/projects/job-market-agent/data/daily-brief.md` | Every Job Market Daily cron-health fix and 10AM film review until next ok run | active |
...
| Passive-income fallback is mistaken for a true no-build verdict | If `memory/passive-income/YYYY-MM-DD-strategist.md` contains `DEGRADED FALLBACK`, `RECOVERED DEGRADED RUN`, or a Scout file contains `DETERMINISTIC HANDOFF`, the Sunday digest must say the full validation did not run and a Mission Control validation task must exist before the next Sunday. Do not count fallback `BUILD: 0` as a real strategist recommendation. | `scripts/passive_income_strategist_delivery_guard.py` + `scripts/passive_income_scout_handoff.py` + Passive Income Scout/Strategist cron payloads + MC task gate | Every Sunday passive-income digest + 10AM film review after fallback delivery | active |
...
- 2026-06-22 film review: 2026-06-21 Health Check-in repeated the fallback-mask pattern again. Cron status/delivery looked ok, but the summary said the real check-in was not sent because the runtime lacked filesystem access. The intended Evening Check-in was manually sent once to numeric Telegram as message `24341`, `health/pending-checkin.json` was repaired, and the live cron payload now has `toolsAllow=["exec","message"]`, uses both paths, and has timeout 120s. Existing active check remains the owner surface; verify the next 21:00 run before closing the pattern.
- 2026-06-22 Night Autonomy: restored the Health Check-in cron allow-list to `["read","write","exec","message"]` and explicit numeric Telegram delivery, then ran `openclaw cron run 6be7f564-5cec-47e7-b67c-9b2fcc3ed8de --wait --expect-final --wait-timeout 3m --timeout 120000`. The run still finished `ok` only through fallback delivery and reported the required `exec` filesystem tool unavailable. Mission Control task `j572zr0zcfky50mj8cjqg045c5897xhy` now tracks the approval-required cron prompt/tooling rewrite or runtime tool exposure fix.
- 2026-06-22 late repair: after JT approved proceeding, added `scripts/health_checkin_cron.py` and `scripts/test_health_checkin_cron.py`, converted the Health Check-in cron to a main-session `systemEvent` that calls the helper instead of relying on isolated filesystem tools, verified duplicate-skip for 2026-06-22, cleared the manual-test `runningAtMs` state through the approved gateway restart script, and closed MC task `j572zr0zcfky50mj8cjqg045c5897xhy`. Next production verification is the 2026-06-23 21:00 run.
...
- **Check:** First verify live directive state. If July directives still list crypto under refused scope, use `openclaw cron list --all --json` to confirm `eve-crypto-morning-008` is disabled with no next run, then log the intentional disablement and do not rerun, enable, alert, spend X/API, or pull markets. If the job is enabled, verify latest run final summary includes explicit `telegram_message_sent=true` and `summary_text`, or manually inspect `/Users/jtsomwaru/projects/crypto-agent/data/telegram-summary.txt` and resend it via Telegram.
...
- **Check:** Verify the workflow loads `memory/content/jt-voice-profile.md` and `memory/content/jt-voice-evidence-corpus.md`, then runs `python3 scripts/jt_voice_guard.py [draft-file] --platform linkedin|x` for serious saved drafts. Public JT-written inventory can enrich the model, but the v1 local evidence corpus is now the active standard.
- **Fail condition:** Content agents claim they captured JT's voice without loading the evidence corpus, skip the pre-send voice guard, or deliver drafts with generic AI hooks, weak specificity, stale reveal framing, internal-process topics, false agency, narrator-from-distance phrasing, vague declaratives, generic Wh-opener setups, pull-quote endings, or high-confidence passive voice.
- **Command:** Run `PYTHONPATH=scripts python3 -m unittest scripts/test_jt_voice_guard.py` after changing voice guard logic.
- **Owner surface:** `memory/content/jt-voice-profile.md`, `memory/content/jt-voice-evidence-corpus.md`, `scripts/jt_voice_guard.py`, `skills/content-generation/SKILL.md`, content-generation workflow, and Content Voice Sprint task.
...
## Passive-income scout/strategist handoff check
- **Trigger:** Any passive-income Scout, Strategist, or delivery-guard failure, or any edit to `agents/passive-income-scout/AGENT.md`, the Scout/Strategist cron payloads, `scripts/passive_income_scout_handoff.py`, `scripts/passive_income_handoff_check.py`, or `scripts/passive_income_strategist_delivery_guard.py`.
- **Check:** Scheduled passive-income crons must keep deterministic artifact creation off the LLM critical path. The Scout cron should run `scripts/passive_income_scout_handoff.py` and stop after pre-scout/pre-strategist checks. The scheduled Strategist cron should run `scripts/passive_income_strategist_delivery_guard.py --send`; full deep validation is manual/optional when the lane is active. Delivery markers must be fresher than the strategist report they prove. If deterministic fallback runs two weeks in a row, create or update one MC validation task and label the next digest as degraded, not a true no-build verdict.
- **Fail condition:** Scout or Strategist cron reads broad history, does live research, burns 100K+ tokens, relies on an LLM final confirmation before writing artifacts, labels stale-but-readable inputs invalid, writes `INCOMPLETE` when a compact degraded artifact could be produced, uses pseudo-command syntax for searches/checks, accepts a stale delivery marker after overwriting a strategist report, leaves `pre-scout`, `pre-strategist`, or `post-strategist` failing for the current date, or allows two consecutive fallback weeks to pass without a real validation task/owner.
- **Command:** `python3 scripts/test_passive_income_scout_handoff.py`; `python3 -m py_compile scripts/passive_income_scout_handoff.py scripts/passive_income_handoff_check.py scripts/passive_income_strategist_delivery_guard.py`; after rerun, `python3 scripts/passive_income_handoff_check.py --mode pre-scout`, `python3 scripts/passive_income_handoff_check.py --mode pre-strategist`, and `python3 scripts/passive_income_handoff_check.py --mode post-strategist` must return `PASSIVE_INCOME_HANDOFF_OK`; Scout and Strategist cron `lastRunStatus` must be `ok` after manual verification.
- **Owner surface:** `scripts/passive_income_scout_handoff.py`, `scripts/test_passive_income_scout_handoff.py`, `scripts/passive_income_handoff_check.py`, `scripts/passive_income_strategist_delivery_guard.py`, `agents/passive-income-scout/AGENT.md`, passive-income Scout/Strategist cron payloads, passive-income strategist delivery guard cron.

## Cron list null-heavy status check
- **Trigger:** Heartbeat cron health checks, especially after `openclaw cron list --json` shows enabled jobs with `status: error` but null `lastRunStatus`, null `consecutiveErrors`, and null `nextRunAtMs`.
...
- **Fail condition:** The Morning Brief aborts, misses Telegram delivery, or records a failed turn because the agent wrote pseudo-command text such as `run python3 ... (agent)` or treated a Nash skip as a whole-brief blocker.
...
## Active Check — Preserve relative paths in bulk archives
- **Trigger:** Any move/archive of two or more files, especially repeated names such as `SKILL.md.bak`.
- **Check:** Resolve all destinations before mutation and fail if any destination repeats. Preserve each source's relative parent path. Afterward compare source/destination counts, total bytes, and hashes.
- **Failure action:** Stop; restore exact sources from a verified repository or backup; remove only the temporary failed archive through Trash; do not continue the intended cleanup.
- **Owner surface:** maintenance shell commands and Weekly Systems Review cleanup actions.
- **Added/verified:** 2026-09-07 after same-basename collision; 11 tracked backups restored and clean status verified.

## Daily Send Sheet started-marker ordering check
- **Trigger:** Every Daily Send Sheet run after the mandated first source read.
- **Check:** Read `memory/job-state/daily-send-sheet.md` and write the current run's started marker before any further evidence collection. The completed run record must preserve the command/state ordering.
- **Fail condition:** Any evidence collection beyond the mandated first source read occurs before the started marker is written.
