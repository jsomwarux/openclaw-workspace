...
| Spanish lesson cron state update fails after delivery | After Spanish Daily Lesson runs, verify latest run delivery fields and `spanish/state.json` parse/date/day increment; if delivery succeeded but state update failed, patch state manually and update cron prompt/tool rule. | Spanish Daily Lesson cron payload + `spanish/state.json` + `agents/autoresearch/checklists/spanish-daily-lesson.md` | Daily lesson delivery check + heartbeat on errors | active |
