...
- **10AM film review:** read yesterday's daily note, find one failure/friction point, write a concrete fix to the owner surface, and add/verify a regression check. Also make one targeted proactive improvement unless genuinely clear. Append one line to `memory/training/training-log.md`.
