...
- **Missed 10AM cron check:** verify crypto morning (`eve-crypto-morning-008`) and outreach pipeline (`651fa1da-84d7-44b3-8e10-6a46e1c05cf6`) ran today; fire/alert if missed. Do not rely on the short outreach prefix alone when calling `openclaw cron runs`, because prefix-only checks can return an empty run history. For crypto, confirm the actual recommendation payload was sent, not only metadata.
