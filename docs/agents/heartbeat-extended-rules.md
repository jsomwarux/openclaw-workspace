...
- **Heartbeat command forms:** for OpenClaw cron commands and `scripts/cron_volume_guard.py`, prefix the shell with `PATH="/opt/homebrew/Cellar/node@22/22.22.2_2/bin:$PATH"`. For Mission Control `/api/tasks` jq slices, normalize the object/array response with `(.tasks // .items // .)` before counting, filtering, or mapping. Use executable shell commands only; do not use pseudo `jq ... (agent)` or `openclaw ... (agent)` text in heartbeat/cron instructions.
...
- **Missed 10AM cron check:** verify outreach pipeline (`651fa1da-84d7-44b3-8e10-6a46e1c05cf6`) ran today. For crypto morning (`eve-crypto-morning-008`), first check the live cron state and the July standing directives. If the job is enabled, verify it ran today and confirm the actual recommendation payload was sent, not only metadata; fire/alert if enabled and missed. If the job is disabled under `directives/00-README.md` refused scope, log `crypto morning intentionally disabled` and do not rerun, enable, alert, or spend X API. Do not rely on the short outreach prefix alone when calling `openclaw cron runs`, because prefix-only checks can return an empty run history.
