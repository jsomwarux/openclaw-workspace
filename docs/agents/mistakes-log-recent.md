...
## 2026-08-10 — Claude/Eve Sync Packet Collapsed Distinct Client States

- **Failure:** The Claude/Eve sync packet marked Karen referral-ready, collapsed Gil/Aya/Maiky into one stale Aya state, treated Altmark FTE/NewCo as a watch item with Ron/Yair, routed DHCR too close to Yair compensation context, called Mission Control's 251 active tasks a healthy state, and carried dollar figures without consistently tying collected cash to the payments ledger.
- **Root cause:** I over-trusted the July 16 compact canonical summary and `memory/pipeline.jsonl` forecast state after direct source reads, instead of treating newer JT corrections and the Mission Control payments ledger as the controlling source for collected cash and live opportunity state.
- **Guardrail/rule:** Sync packets must split people/entities/opportunities before summarizing; collected cash must cite the payments ledger, while proposal/balance/pipeline figures are marked forecast/UNVERIFIED. Mission Control active-task counts cannot be called healthy until a stale-backlog/due-date triage runs.
...
## 2026-09-14 — Capability marker was returned only after a potentially unsafe legacy mutation
- **Failure:** The first create-only capability handshake used `POST /api/tasks?mode=create-only` and checked `writeMode` only in the response; an older deployment could ignore the unknown query parameter, execute its ordinary keyed upsert, and overwrite existing task state before the client detected the missing marker.
- **Root cause:** Capability negotiation was placed after the side-effect boundary. The design treated an additive response field as sufficient without accounting for legacy handlers that accept unknown query parameters and mutate before responding.
- **Guardrail/rule:** Any write-safety capability must fail closed before side effects through a distinct route, method, or protocol version that unsupported servers cannot dispatch to a mutable legacy handler. Never use a post-write response marker as the sole capability probe.
- **Regression check:** `task-create-only-route.test.ts` must prove the dedicated route returns the marker for created and existing outcomes, rejects missing dedupe keys without mutation, and that the legacy dynamic task route exports no POST handler; ordinary task-response tests must remain green.
- **Owner surface updated:** `mission-control/app/api/tasks/create-only/route.ts`, `mission-control/lib/mission-control/task-create-only-post.ts`, `mission-control/lib/mission-control/task-create-only-route.test.ts`, implementation/review artifacts, and this Mistakes Log entry.
- **Verification/date:** 2026-09-14 — builder run recorded 14 focused tests and 142 full-suite tests passing, TypeScript exit 0, and the isolated build listing `/api/tasks/create-only` as a distinct route; fresh-context verdict remains pending.

## 2026-09-15 — Reintroduced retired 90 Day Playbook gates in the OS map
- **Failure:** The eight-lane AI Workflow Growth OS map described the November 17 freeze and invoice-within-90-days test as current authority for app, passive-income, profile, and website work after JT had explicitly retired that playbook on September 8.
- **Root cause:** The map reused current-action language from the older v3.7 plan and archived memory without reconciling it against the later, higher-precedence retirement recorded in `USER.md` and `MEMORY.md`.
- **Guardrail/rule:** Before any strategy or system-status map, search for later corrections to dated mandates and label retired mandates historical. Current deferrals must cite the live capacity rule and the lane-specific missing input, conversion surface, bounded test, or capacity constraint.
- **Regression check:** The current sequence addendum must name rule 13 as the only capacity gate, record this week's five buyer messages, and contain no current-action November 17 or invoice-within-90-days gate; profile/homepage must be active rather than deferred.
- **Owner surface updated:** `MEMORY.md`, `docs/memory/MEMORY-full.md`, `memory/2026-09-15.md`, `docs/superpowers/plans/2026-09-15-ai-workflow-growth-os-sequence-addendum.md`, and this Mistakes Log entry.
- **Verification/date:** 2026-09-15 — durable state records the September 8 retirement, five cohort-one sends satisfy rule 13 for this week, the sequence addendum makes the seven-field card contract step 5, and the profile/homepage edit card is live in Mission Control.

## 2026-09-15 — Card-contract API validated malformed input after the HTTP boundary
- **Failure:** Fresh review found malformed `exactSteps`, `pasteReadyPrompt`, and `pasteDestination`, invalid JSON, and blank or oversized feedback could escape the intended client-error path and surface as HTTP 500 instead of a safe 400.
- **Root cause:** The generic task route parsed JSON outside its error boundary, additive card fields were allowlisted without structural validation, and feedback length/blank checks ran only inside the Convex mutation after the API accepted the request.
- **Guardrail/rule:** Every public API route must parse and structurally validate caller-owned input before invoking Convex or another dependency; downstream validators remain defense in depth, not the first client-error boundary.
- **Regression check:** `task-route-validation.test.ts` sends malformed JSON, a non-array `exactSteps`, blank feedback, and 4,001-character feedback and requires HTTP 400 without a Convex call; admission tests separately reject every malformed card-field type.
- **Owner surface updated:** Mission Control task route, task-admission validator/tests, feedback parser/tests, builder claim receipt, and this Mistakes Log entry.
- **Verification/date:** 2026-09-15 — RED reproduced all three HTTP failure classes; GREEN focused run passed 15/15, full Mission Control run passed 204/204, TypeScript exited 0, and the 41-route production build completed. A fresh non-builder independently tested exact repair commit `2b7da9520e7dc1edcbdaceba3563cf1e21cf4293` with 11 hostile HTTP probes, 82 focused tests, the 204-test full suite, TypeScript, build, credential scan, and outreach immutability checks, returning `CONFIRM` with no failures.

## 2026-09-15 — Gate receipt accepted unknown fields after recomputed hash
- **Failure:** Fresh verification proved a committed gate receipt could add the hidden channel value and caller-controlled identity metadata, recompute its receipt hash, and still pass owner attestation and draft finalization.
- **Root cause:** The artifact and hash bindings checked integrity, but receipt validation required only a subset of keys. Unknown fields were ignored, so integrity was mistaken for an exact data contract.
- **Guardrail/rule:** Every security-sensitive receipt must enforce an exact field allowlist before freshness checks, owner/network calls, or hash-bound downstream reconstruction. A recomputed hash never licenses an expanded schema.
...
## 2026-09-17 — Cohort-two pilot preflight missed an external read path and zero-item termination
- **Failure:** The first authorized real-data pilot needed a process-local allowlist expansion at N04, then execution 1716 stopped silently at N08 when valid empty HPD contacts arrays emitted zero n8n items. n8n reported `success` despite producing no cohort, artifacts, completion marker, or state update.
- **Root cause:** D3 validation covered the cohort project root but not the daily preflight directory N04 reads. The fixture harness invoked N09 directly with `[]`, so it tested empty business data but not n8n's structural zero-item propagation between N08 and N09.
- **Guardrail/rule:** Release preflight must enumerate every absolute read/write root from the generated workflow, and every external read where an empty result is valid must preserve an explicit result item through the real n8n transport. A successful execution status is not completion evidence without the terminal artifact contract.
- **Regression check:** A runtime-shape capture against n8n 2.14.1 must prove the empty contacts response shape; a test with nonempty N07/N07B and empty N08 must reach N09/seed-roster handling and an explicit terminal outcome; the documented D3 allowlist must contain all three required roots.
- **Owner surface updated:** `tasks/lessons.md` lesson 163, `CLAUDE.md`, `MEMORY.md`, `memory/2026-09-17.md`, Mission Control card `j5781atgn16jeneassmz5hd3td8ejck4`, and the bounded Claude repair prompt.
- **Verification/date:** 2026-09-17 — execution 1716 proved N07=400, N07B=2 chunks, N08=0 items, no downstream nodes, no artifacts, and unchanged state hashes; repair verification remains pending.

## 2026-09-23 — Cohort-two release reconciled only one of two legacy state schemas
- **Failure:** After the explicit ledger recovery passed and was applied, pilot execution 1718 stopped at N03 with `cursor-schema-version-unexpected: undefined`; the pristine `frame-cursor.json` had also been created before its required `schema_version` was added.
- **Root cause:** Release preflight compared the live ledger to its current contract after the first failure but did not compare every persisted state file against the current bootstrap/N03 schema. The repair was artifact-local while the release diagnosis needed to be state-set-wide.
- **Guardrail/rule:** Whenever a persisted-state schema mismatch is found, inventory every sibling state file against current bootstrap bytes and runtime schema before resuming. Recover each known legacy artifact only through a separate exact-byte, human-approved command; never infer that the first repaired file was the only stale artifact.
- **Regression check:** The cursor recovery suite must prove the exact 105-byte legacy cursor is rejected by N03, the exact 147-byte recovered cursor is accepted, all advanced/malformed/value-equal variants are refused untouched, normal bootstrap never migrates it, and the fully bootstrapped cohort suite runs with zero skips before merge. Before the next pilot, hash and schema-check ledger, carryover, and cursor together.
- **Owner surface updated:** `docs/agents/mistakes-log-recent.md`; exact Claude controller prompt `memory/prompts/claude-explicit-frame-cursor-recovery-2026-09-23.md`; cursor recovery implementation/review PR pending.
- **Verification/date:** 2026-09-23 — execution 1718 stopped at N03 before public fetches/model calls; ledger remained at recovered SHA `b2a02ff8…`, carryover remained empty/current, cursor remained the pristine 105-byte legacy SHA `df4e499…`, and no cohort output files were created.

## 2026-09-23 — Claude handoff event was claimed but never delivered to Eve
- **Failure:** The two-page Claude controller wrote a valid `COMPLETE` handoff and told JT that Eve had been notified, but the event never appeared in the gateway logs or the active Telegram session, forcing JT to relay the terminal line manually.
- **Root cause:** The controller used `openclaw system event` without an explicit session key and did not request or verify a final JSON acknowledgment, so default routing and silent non-delivery were treated as success.
- **Guardrail/rule:** Every Claude controller prompt must include Eve's exact session key and send with `--session-key`, `--expect-final`, and `--json`; exit 0 plus a successful JSON response is required before notification can be claimed.
- **Regression check:** The next Claude lane must deliver its completion into `agent:main:telegram:direct:6608544825` without JT relaying it, and its terminal result must include the verified notification outcome; a failed delivery retries once and then reports `BLOCKED_NOTIFICATION`.
- **Owner surface updated:** Applied `claude-lane-controller` skill proposal `claude-lane-controller-20260923-679d8c17fa`; future controller prompts inherit the exact-session delivery contract.
- **Verification/date:** 2026-09-23 — the missing event text was absent from gateway logs and session history while the handoff file was present; the updated live skill now requires exact-session acknowledged delivery.

## 2026-09-23 — Declared remote SSH viable without proving the Tailscale data plane
- **Failure:** Told JT the Mac Mini was ready for remote SSH because Tailscale listed the node online and port 22 was open, but the laptop connection timed out.
- **Root cause:** The preflight checked control-plane registration and the local SSH listener but did not test peer-to-peer Tailscale reachability or TCP 22 through the tailnet. Tailscale's control plane remained online while its MagicSock IPv4 receive loop had failed.
- **Guardrail/rule:** Never declare remote access ready from node status or a local listener alone. Require a clean Tailscale health report, a successful `tailscale ping` to the intended peer, and a TCP 22 connection over the tailnet.
- **Regression check:** From the Mini, `tailscale status` must not report `MagicSock function ReceiveIPv4 is not running`; `tailscale ping macbook-pro-6` must succeed. From the laptop, `nc -vz 100.118.74.7 22` and then `ssh jtsomwaru@jts-mac-mini.tailaf2fd2.ts.net` must connect.
- **Owner surface updated:** `docs/tools/TOOLS-full.md` Mac Mini Remote SSH preflight and this Mistakes Log entry.
- **Verification/date:** 2026-09-23 — localhost TCP 22 succeeded and macOS firewall allowed SSH, while Tailscale reported the MagicSock receive failure, `tailscale ping macbook-pro-6` timed out, and peer SSH timed out; root cause isolated to the Mini's Tailscale data plane.

## 2026-09-23 — Cohort-two pilot used UTC date against a New York-dated exclusion artifact
- **Failure:** Authorized expanded-roster pilot execution 1723 failed at N04 because N03 requested `2026-09-24-script-first-preflight.json` even though it was still September 23 in New York and the freshly generated exclusion artifact was correctly named `2026-09-23-script-first-preflight.json`.
- **Root cause:** N03 derived `run.as_of` with `startedAt.slice(0, 10)`, binding a New York business workflow to the UTC calendar date. The producer uses the Mac Mini's New York date, so the producer and consumer diverge every evening after UTC midnight.
- **Guardrail/rule:** Every date-keyed producer/consumer pair must share one explicit IANA business timezone. Preserve event timestamps in UTC, but derive business dates and date-keyed paths with the configured workflow timezone; never infer business date by slicing a UTC timestamp or subtracting a fixed offset.
- **Regression check:** At `2026-09-24T02:11:12Z`, N03 must produce `as_of=2026-09-23` and the September 23 exclusion path for `America/New_York`; at `2026-09-24T04:00:00Z`, it must produce September 24. Tests must also cover EST/EDT boundaries, invalid timezone refusal before N04, and equality between configured and workflow timezones.
- **Owner surface updated:** `proofs/2026-09-23/cohort-two-expanded-roster-pilot-1723/receipt.md`, `memory/prompts/claude-cohort-two-business-date-repair-2026-09-23.md`, this Mistakes Log entry; n8n-agent code/lessons remain pending the reviewed repair.
- **Verification/date:** 2026-09-23 — execution 1723 persisted `error` at N04 with requested path dated September 24; no public research/model call or output directory occurred, and cursor/ledger/carryover hashes remained exactly unchanged.
