...
## Active Clients
- **Altmark Group:** priority paid client. Dedicated PC installed in office; insurance expiration workflow is live in production, working after minor updates, and final 50% paid. Rent delinquency workflow initial 50% paid and remains the top proof/revenue gate. 2026-05-29 synthetic dry-run passed: 8 rows → 1 included, 4 manual review, 1 excluded, 2 cleanup, and 0 tenant-facing drafts from sensitive/cleanup rows. Next gate: redacted Altmark source export, source report path, refresh cadence, named output reviewer, and confirmed exception rules. DHCR Lease Renewal Phase 1 assets sequence after rent delinquency acceptance unless Altmark explicitly reprioritizes. Keep proof capture privacy-safe; public naming requires accepted client output plus permission/anonymization.
...
- App Marketing/ReelFarm reset, swipe-file requirements, and Sports GM/@dynastyjig content rules are archived in `docs/memory/current-context-details-2026-05-27.md` and `docs/memory/sports-gm-content-system-current.md`.
...
- 2026-05-29 xhigh audit: current non-coding AI enablement roles are converging around AI operating systems, not generic automation: intake/front door, connectors/MCP, eval harnesses, data lineage, governance, Champions/adoption, rollback standards, and ROI reporting. Job-derived actions must consolidate into existing proof lanes when possible instead of creating one new demo per JD. Current H.I.G. Capital signal maps to a spec-first `DealDesk AI Control Plane` / ACE Readiness Sprint, but Altmark proof remains the higher-priority revenue/proof gate. Missing upgrade completed: `~/projects/job-market-agent/data/role-to-build-matrix.md` is now the canonical recurring system for mapping each 18+/25 role to hiring pain, proof gap, best move, mapped artifact, build/demo decision, consulting angle, priority vs Altmark, and Mission Control action before creating tasks.
...
- 2026-05-29: four enabled crons show one-error stale states after useful work or duplicate-risk output: Job Market Daily, Skills & API Researcher, Viral Post Swipe, and `t3-cold-hook`. Job Market, Skills Researcher, and Viral Swipe payloads/prompts were hardened against inline Python/heredoc verification; `scripts/mission_control_task_gate.py` is the reusable task gate. Do not rerun these solely to clear stale status.
