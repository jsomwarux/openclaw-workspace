...
- Follow-up A: status=completed - Finding 1 follow-up diagnosed Moonshot as an account/provider billing state, not an OpenAI routing error: OpenClaw reports `moonshot:default` as `disabled:billing 2h`, and gateway logs show Moonshot 429 insufficient-balance/account-suspended errors. No fallback was applied because JT-side Moonshot billing must be fixed first; after JT confirms recharge/reactivation, rerun one smoke test and apply the original Phase 2A fallback spec unchanged. If Moonshot remains unsuitable, approved alternative is OpenRouter Gemini Flash class with $5/month fallback cap and $3 alert after smoke. Phase 2B cooldown recovery was already present; synthetic dry-run logged the expected trigger. Baseline delivery was 48/76 delivered, 63.2%. Recompute delivery rate 7 days after fallback lands using `python3 scripts/recompute_delivery_rate.py --days 7 --label fallback-plus-7 --json`; Mission Control follow-up task: `j572j139cb89jkpf91h7abz24988f1hp`.
- Follow-up B: status=completed - Post-implementation sweep verified restart persistence, content prompt appendix prefixes, Night Autonomy live state, 12 removed zombies absent, and old night jobs disabled. Fixed AGENTS autonomy-lane carve-outs, Night Autonomy one-artifact/blocker contract, MEMORY regrowth/bootstrap notes, cron snapshot hash dedupe, and residual redactions. Blockers: Nash Groq warm-up cannot deliver because runtime allowlist rejects `groq/llama-3.3-70b-versatile`; Convex local backend exposes `--instance-secret` by upstream CLI argv, so moving that process secret to env/keychain is not safely fixable without changing Convex runtime approach.
